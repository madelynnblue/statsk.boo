...
pub async fn handle(
    ...
    let home_penalties: Vec<&PlayerPenalties> = player_penalties.iter().filter(|p| p.side == "home").collect();
    let away_penalties: Vec<&PlayerPenalties> = player_penalties.iter().filter(|p| p.side == "away").collect();
...
fn group_penalties(penalties: &[Penalty], home_names: &HashMap<String, String>, away_names: &HashMap<String, String>) -> Vec<PlayerPenalties> {
    let mut groups: HashMap<(String, String), Vec<&Penalty>> = HashMap::new();
    for p in penalties {
        groups.entry((p.side.clone(), p.number.clone())).or_default().push(p);
    }
    let mut result: Vec<PlayerPenalties> = groups.into_iter().map(|((side, number), pens)| {
        let foul_out = pens.iter().any(|p| p.foul_out);
        let total = pens.len();
        let penalty_items: Vec<PenaltyItem> = pens.iter().map(|p| PenaltyItem {
            code: p.code.clone(),
            period: p.period,
            jam: p.jam,
        }).collect();
        let name = match side.as_str() {
            "home" => home_names.get(&number).cloned().unwrap_or_else(|| number.clone()),
            _      => away_names.get(&number).cloned().unwrap_or_else(|| number.clone()),
        };
        PlayerPenalties { side, number, name, total, foul_out, penalties: penalty_items }
    }).collect();
